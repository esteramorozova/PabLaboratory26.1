...
public class IdentityDbSeeder : IDataSeeder
{
    ...
    private async Task SeedRolesAsync()
    {
        ...
        foreach (var role in roles)
        {
            ...
            if (!result.Succeeded)
                _logger.LogError("Błąd tworzenia roli {Role}: {Errors}",
    ...
    private async Task SeedUsersAsync()
    {
        var users = new[]
        {
            new SeedUser("F5BADE14-6CC8-42A2-9A44-9842DA2D9280",
                "admin@crm.pl", "Adam", "Administrator", "IT", "Admin@123!", UserRole.Administrator),
            new SeedUser("93A7FFDD-057F-4021-9C68-FE06951FFA65",
                "jan.kowalski@crm.pl", "Jan", "Kowalski", "Sales", "Manager@123!", UserRole.SalesManager),
            new SeedUser("3D4769E2-1C75-43E1-A5BB-1F71C68E9F57",
                "anna.nowak@crm.pl", "Anna", "Nowak", "Sales", "Sales@123!", UserRole.Salesperson),
            new SeedUser("0E136AB2-1A6A-4A16-938D-84DFB0F64BBA",
                "piotr.wisniewski@crm.pl", "Piotr", "Wiśniewski", "Sales", "Piotr123!", UserRole.Salesperson),
            new SeedUser("76B253D6-C16C-470A-943C-92F314A090F2",
                "maria.wojcik@crm.pl", "Maria", "Wójcik", "Support", "Support@123!", UserRole.SupportAgent),
            new SeedUser("E90A39C9-9CE2-400A-8A7B-8CF300D3B292",
                "tomasz.kaminski@crm.pl", "Tomasz", "Kamiński", "Management", "Readonly@123!", UserRole.ReadOnly)
        };
    ...
    private async Task CreateUserAsync(SeedUser seedUser)
    {
        ...
        {
            _logger.LogError("Błąd tworzenia użytkownika {Email}: {Errors}",
                user.Email, FormatErrors(createResult));
            return;
        }
        ...
        {
            _logger.LogError("Błąd przypisania roli {Role} dla {Email}: {Errors}",
                seedUser.Role, seedUser.Email, FormatErrors(roleResult));
            return;
        }

        _logger.LogInformation("Utworzono użytkownika {Email} z rolą {Role}.",
